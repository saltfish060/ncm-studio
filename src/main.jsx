...
const browserMock = {
  chooseFiles: async () => [], chooseFolder: async () => null, filePathsFromDrop: () => [], fileDetails: async () => [],
  getSettings: async () => ({ ...EMPTY_SETTINGS, outputDirectory: 'C:\\Users\\User\\Music\\NCM Studio' }),
  saveSettings: async (settings) => settings, scan: async () => ({ files: [], roots: 0 }), convert: async () => ({ results: [] }), cancel: async () => true,
  openFolder: async () => '', openLicenses: async () => '', setTheme: () => {}, onScanProgress: () => () => {}, onConvertProgress: () => () => {}
};
const api = window.ncmStudio || browserMock;
...
function Sidebar({ view, setView, count, historyCount, onNavigate }) {
  return <aside className="sidebar">
    <div className="brand"><span className="brand-mark"><Music2 size={19} strokeWidth={2.4}/></span><span>NCM Studio</span></div>
    <nav>
...
function SettingsPanel({ settings, setSettings, save, saved }) {
  ...
  return <main className="settings-page">
    ...
    <section className="settings-section"><h3>输出</h3>
      ...
      <div className="field"><span>音频格式</span><div className="format-options">
      ...
      {settings.format === 'mp3' && <label className="field inline"><span>MP3 音质</span><select value={settings.mp3Bitrate} onChange={(e) => set('mp3Bitrate', e.target.value)}><option value="320k">320 kbps</option><option value="256k">256 kbps</option><option value="192k">192 kbps</option><option value="128k">128 kbps</option></select></label>}
    </section>
    ...
    <section className="settings-section"><h3>关于</h3><div className="about-grid"><div><strong>NCM Studio</strong><small>版本 1.0.0</small></div><div><strong>制作</strong><small>3mber</small></div><div><strong>运行方式</strong><small>完全本地处理，不联网</small></div></div><p className="about-note">个人学习与技术研究项目。代码使用 MIT 许可，随包内含 FFmpeg（GPL-3.0-or-later）。</p><p className="about-note about-warn">请仅转换你本人已合法获得的音频文件，勿传播或商用；本程序与网易云音乐、网易公司无关联。</p><button className="secondary about-button" onClick={() => api.openLicenses()}><FileText size={16}/>查看开源许可</button></section>
